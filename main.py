...
class Trainer(object):
    def __init__(self, data_config):
        ...
        self.model = REDGL(self.n_users, self.n_items, self.emb_dim, self.weight_size, self.mess_dropout, image_feats, text_feats)
    ...
    def test(self, users_to_test, is_val):
        self.model.eval()
        with torch.no_grad():
            ua_embeddings, ia_embeddings, _ = self.model(self.norm_adj, build_item_graph=True)
    ...
    def train(self):
        ...
        for epoch in (range(args.epoch)):
            t1 = time()
            loss, mf_loss, emb_loss, reg_loss, pca_loss = 0., 0., 0., 0., 0.
            n_batch = data_generator.n_train // args.batch_size + 1
            sample_time = 0.
            ...
            for idx in (range(n_batch)):
                ...
                ua_embeddings, ia_embeddings, batch_pca_loss = self.model(self.norm_adj, build_item_graph=build_item_graph)
                ...
                batch_loss = batch_mf_loss + batch_emb_loss + batch_reg_loss + batch_pca_loss
            ...
            perf_str = 'Epoch %d [%.1fs]: train==[%.5f=%.5f + %.5f + %.5f]' % (
                epoch, time() - t1, loss, mf_loss, emb_loss, pca_loss)
